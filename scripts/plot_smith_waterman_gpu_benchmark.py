...
BENCHMARK_CSV_PATH = Path("benchmarks/smith_waterman_gpu_benchmark_results.csv")
CHART_OUTPUT_DIRECTORY = Path("assets/benchmark_charts/smith_waterman_gpu")
...
def load_rows(csv_path: Path) -> list[dict[str, str]]:
    if not csv_path.exists():
        raise FileNotFoundError(f"Smith-Waterman GPU benchmark CSV not found: {csv_path}")
    with csv_path.open("r", encoding="utf-8", newline="") as csv_file:
        rows = list(csv.DictReader(csv_file))
    if not rows:
        raise ValueError(f"Smith-Waterman GPU benchmark CSV has no data rows: {csv_path}")
    return rows
...
def save_grouped_time_chart(rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    plt.title("Smith-Waterman CPU vs GPU Total Time")
    plt.xlabel("Workload")
    plt.ylabel("Time (ms)")
...
def save_copy_kernel_breakdown(rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    plt.title("Smith-Waterman GPU H2D, Kernel, and D2H Breakdown")
    plt.xlabel("Workload")
    plt.ylabel("Time (ms)")
    plt.xticks(x_values, labels, rotation=45, ha="right")
...
def save_scaling_by_sequence_length(rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    plt.title("Smith-Waterman GPU Scaling by Sequence Length")
...
def main() -> None:
    ...
    save_grouped_time_chart(rows, CHART_OUTPUT_DIRECTORY / "sw_gpu_cpu_vs_gpu_time.png")
    save_bar_chart(
        rows,
        "Smith-Waterman GPU Kernel Speedup",
        "Speedup vs CPU",
        "kernel_speedup",
        CHART_OUTPUT_DIRECTORY / "sw_gpu_kernel_speedup.png",
    )
    save_bar_chart(
        rows,
        "Smith-Waterman GPU Total Speedup",
        "Speedup vs CPU",
        "total_speedup",
        CHART_OUTPUT_DIRECTORY / "sw_gpu_total_speedup.png",
    )
    save_bar_chart(
        rows,
        "Smith-Waterman GPU Total Cells per Second",
        "DP cells per second",
        "cells_per_second_gpu_total",
        CHART_OUTPUT_DIRECTORY / "sw_gpu_cells_per_second.png",
        use_log_scale=True,
    )
    save_copy_kernel_breakdown(
        rows,
        CHART_OUTPUT_DIRECTORY / "sw_gpu_h2d_kernel_d2h_breakdown.png",
    )
    save_scaling_by_sequence_length(
        rows,
        CHART_OUTPUT_DIRECTORY / "sw_gpu_scaling_by_sequence_length.png",
    )

    print(f"Smith-Waterman GPU benchmark charts saved to: {CHART_OUTPUT_DIRECTORY}")
