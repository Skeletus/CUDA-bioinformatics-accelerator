...
CHART_OUTPUT_DIRECTORY = Path("assets/benchmark_charts/smith_waterman_cpu")
...
def load_rows(csv_path: Path) -> list[dict[str, str]]:
    if not csv_path.exists():
        raise FileNotFoundError(f"Smith-Waterman benchmark CSV not found: {csv_path}")
    with csv_path.open("r", encoding="utf-8", newline="") as csv_file:
        rows = list(csv.DictReader(csv_file))
    if not rows:
        raise ValueError(f"Smith-Waterman benchmark CSV has no data rows: {csv_path}")
    return rows
...
def save_complexity_growth_chart(rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    plt.title("Smith-Waterman CPU Complexity Growth")
...
def main() -> None:
    ...
    save_bar_chart(
        rows,
        "Smith-Waterman CPU Time by Workload",
        "CPU time (ms)",
        "cpu_time_ms",
        CHART_OUTPUT_DIRECTORY / "sw_cpu_time_by_workload.png",
        use_log_scale=True,
    )
    save_bar_chart(
        rows,
        "Smith-Waterman CPU Cells per Second",
        "DP cells per second",
        "cells_per_second",
        CHART_OUTPUT_DIRECTORY / "sw_cells_per_second.png",
        use_log_scale=True,
    )
    save_bar_chart(
        rows,
        "Smith-Waterman Average Time per Pair",
        "Average time per pair (ms)",
        "average_time_per_pair_ms",
        CHART_OUTPUT_DIRECTORY / "sw_average_time_per_pair.png",
        use_log_scale=True,
    )
    save_complexity_growth_chart(
        rows,
        CHART_OUTPUT_DIRECTORY / "sw_complexity_growth.png",
    )

    print(f"Smith-Waterman CPU benchmark charts saved to: {CHART_OUTPUT_DIRECTORY}")
