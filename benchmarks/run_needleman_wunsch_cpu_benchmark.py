...
def build_row(num_pairs: int, sequence_length: int, values: dict[str, str]) -> dict[str, str | int | float]:
    ...
    return {
        "algorithm": values.get("ALGORITHM", "needleman_wunsch_cpu"),
        "num_pairs": num_pairs,
...
def main() -> int:
    ...
    for sequence_length in SEQUENCE_LENGTHS:
        for num_pairs in NUM_PAIRS_VALUES:
            dataset_path = dataset_directory / f"nw_synthetic_pairs_len{sequence_length}_n{num_pairs}.txt"
            output_path = result_directory / f"needleman_wunsch_cpu_len{sequence_length}_n{num_pairs}.csv"
    ...
    print(f"Needleman-Wunsch CPU benchmark results saved to: {project_root / BENCHMARK_CSV_PATH}")
    if failed_workloads:
