...
def compile_gpu(project_root: Path) -> Path:
    binary_path = executable_path(project_root / "smith_waterman_gpu")
    run_command(
        [
            "nvcc",
...
def build_row(
    ...
    return {
        ...
        "cells_per_second_gpu_kernel": safe_divide(
            float(total_cells_computed),
            gpu_kernel_time_ms / 1000.0,
        ),
        "cells_per_second_gpu_total": safe_divide(
            float(total_cells_computed),
            gpu_total_time_ms / 1000.0,
        ),
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark Smith-Waterman CPU and GPU implementations.")
    parser.add_argument(
        "--quick",
        action="store_true",
...
def main() -> int:
    ...
    for sequence_length in sequence_lengths:
        for num_pairs in num_pairs_values:
            dataset_path = dataset_directory / f"sw_gpu_synthetic_pairs_len{sequence_length}_n{num_pairs}.txt"
            cpu_output_path = result_directory / f"smith_waterman_cpu_len{sequence_length}_n{num_pairs}.csv"
            gpu_output_path = result_directory / f"smith_waterman_gpu_len{sequence_length}_n{num_pairs}.csv"
    ...
    print(f"Smith-Waterman GPU benchmark results saved to: {benchmark_csv_path}")
    if failed_workloads:
