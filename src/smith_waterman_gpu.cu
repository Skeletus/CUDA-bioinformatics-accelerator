...
constexpr int MAX_SHARED_SEQUENCE_LENGTH = 64;
...
__device__ int device_max4(int first_value, int second_value, int third_value, int fourth_value) {
    int maximum_value = first_value > second_value ? first_value : second_value;
    maximum_value = maximum_value > third_value ? maximum_value : third_value;
    return maximum_value > fourth_value ? maximum_value : fourth_value;
}

__global__ void smithWatermanBaselineKernel(const char* sequenceA,
    ...
    if (threadIdx.x == 0) {
        for (int row = 0; row <= sequenceLengthA; ++row) {
            dp[row * cols] = 0;
        }
        for (int col = 0; col <= sequenceLengthB; ++col) {
            dp[col] = 0;
        }
        ...
        for (int row = 1; row <= sequenceLengthA; ++row) {
            for (int col = 1; col <= sequenceLengthB; ++col) {
                ...
                const int cellScore = device_max4(0, diagonalScore, upScore, leftScore);
                dp[row * cols + col] = cellScore;
                maximumScore = cellScore > maximumScore ? cellScore : maximumScore;
            }
        }
        scores[pairIndex] = maximumScore;
    }
}

__global__ void smithWatermanWavefrontKernel(const char* sequenceA,
                                             const char* sequenceB,
                                             int* scores,
                                             int numberOfPairs,
                                             int sequenceLengthA,
    ...
    int* dp = sharedMemory;
    int* threadMaxima = dp + matrixCells;
    ...
    for (int diagonal = 2; diagonal <= sequenceLengthA + sequenceLengthB; ++diagonal) {
        ...
        for (int cellOffset = threadIdx.x; cellOffset < cellsOnDiagonal; cellOffset += blockDim.x) {
            ...
            const int cellScore = device_max4(0, diagonalScore, upScore, leftScore);
            dp[row * cols + col] = cellScore;
            localMaximum = cellScore > localMaximum ? cellScore : localMaximum;
        }
        __syncthreads();
    }

    threadMaxima[threadIdx.x] = localMaximum;
    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride /= 2) {
        if (threadIdx.x < stride) {
            const int candidate = threadMaxima[threadIdx.x + stride];
            threadMaxima[threadIdx.x] =
                candidate > threadMaxima[threadIdx.x] ? candidate : threadMaxima[threadIdx.x];
        }
        __syncthreads();
    }

    if (threadIdx.x == 0) {
        scores[pairIndex] = threadMaxima[0];
    }
}
...
ProgramOptions parse_arguments(int argc, char** argv) {
    if (argc < 3) {
        throw std::runtime_error(
            ...
            "[--repetitions N] [--implementation baseline|wavefront] [--summary-only] "
            "[--skip-validation]");
    }
...
Dataset read_dataset(const std::string& input_path) {
    ...
    while (input_file >> first_sequence >> second_sequence) {
        ...
        if (!has_sequence_lengths) {
            ...
            throw std::runtime_error(
                "ERROR: Phase 11 GPU implementation currently requires fixed-length sequence pairs.");
        }
    ...
    if (dataset.sequence_length_a > MAX_SHARED_SEQUENCE_LENGTH ||
        dataset.sequence_length_b > MAX_SHARED_SEQUENCE_LENGTH) {
        throw std::runtime_error(
            "ERROR: Sequence length exceeds MAX_SUPPORTED_SEQUENCE_LENGTH for the Phase 11 shared-memory prototype.");
    }
...
std::vector<int> compute_cpu_reference_scores(const Dataset& dataset, const ProgramOptions& options) {
    ...
    for (const SequencePair& pair : dataset.pairs) {
        cpu_scores.push_back(smith_waterman::computeSmithWatermanScore(
            pair.first_sequence,
...
void launch_kernel(const ProgramOptions& options,
    ...
    const std::size_t shared_memory_bytes =
        options.implementation == "wavefront"
            ? (matrix_cells + THREADS_PER_BLOCK) * sizeof(int)
            : matrix_cells * sizeof(int);

    CUDA_CHECK(cudaEventRecord(kernel_start_event));
    if (options.implementation == "baseline") {
        smithWatermanBaselineKernel<<<dataset.number_of_pairs, THREADS_PER_BLOCK, shared_memory_bytes>>>(
        ...
        smithWatermanWavefrontKernel<<<dataset.number_of_pairs, THREADS_PER_BLOCK, shared_memory_bytes>>>(
            device_first_sequences,
            device_second_sequences,
            device_scores,
            dataset.number_of_pairs,
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "ALGORITHM=smith_waterman_gpu\n";
        ...
        std::cout << "MAX_SUPPORTED_SEQUENCE_LENGTH=" << MAX_SHARED_SEQUENCE_LENGTH << "\n";
        std::cout << "DP_MEMORY_MODE=shared\n";
